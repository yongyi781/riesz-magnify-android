				...
				for (unsigned l = 0; l < M - i + 1; l++) {
					if (k + l == j) {
						val = val + std::complex<double>(comb(i, k)) *
							...
							b[N - i] * pow(2 * fs, (int)i)*pow(-1.0, (int)k);
					}
				}
			}
		}
				...
				for (unsigned l = 0; l < M - i + 1; l++) {
					if (k + l == j) {
						val += std::complex<double>(comb(i, k)) *
							std::complex<double>(comb(M - i, l)) *
							a[D - i] * pow(2 * fs, (int)i)*pow(-1.0, (int)k);
					}
				}
			}
		}
...
void filter_util::computeFilter(double fps, double lowCutoff, double highCutoff, std::vector<double>& filterA, std::vector<double>& filterB)
{
	if (lowCutoff < 0 || highCutoff < 0)
		throw std::invalid_argument("Invalid cutoffs");

    std::vector<double> w;
    w.push_back(lowCutoff / (fps / 2.0));
    w.push_back(highCutoff / (fps / 2.0));
	filter_util::butterBP(1, w, filterA, filterB);
	filterA[1] /= filterA[0];
	filterA[2] /= filterA[0];
}
