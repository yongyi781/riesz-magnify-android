...
JNIEXPORT jlong JNICALL Java_com_yongyi_rieszmagnifyandroid_AndroidBufferUtilities_allocNativeYuvBufferT(
...
JNIEXPORT jboolean JNICALL Java_com_yongyi_rieszmagnifyandroid_AndroidBufferUtilities_freeNativeYuvBufferT(
...
JNIEXPORT jboolean JNICALL Java_com_yongyi_rieszmagnifyandroid_AndroidBufferUtilities_rotateNativeYuvBufferT180(
    JNIEnv *env, jobject, jlong handle) {
    if (handle == 0L) {
        return false;
    }
...
JNIEXPORT jlong JNICALL Java_com_yongyi_rieszmagnifyandroid_AndroidBufferUtilities_lockSurface(
    JNIEnv *env, jobject, jobject surface) {
    return reinterpret_cast<jlong>(LockedSurface::lock(env, surface));
}

JNIEXPORT jlong JNICALL Java_com_yongyi_rieszmagnifyandroid_AndroidBufferUtilities_allocNativeYuvBufferTFromSurfaceHandle(
    JNIEnv *env, jobject, jlong lockedSurfaceHandle) {
...
JNIEXPORT jboolean JNICALL Java_com_yongyi_rieszmagnifyandroid_AndroidBufferUtilities_unlockSurface(
    JNIEnv *env, jobject, jlong lockedSurfaceHandle) {
    if (lockedSurfaceHandle == 0L) {
        return false;
    }
